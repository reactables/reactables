...
import { BaseAbstractControl, BaseArrayControl } from '../Models/Controls';
import { Reducer } from '@hub-fx/core';
import { ControlRef } from '../Models/ControlRef';
import { HubFactory } from '@hub-fx/core';
import { formChange } from '../Actions/Hub2';
...
export const buildHub2Source = <T extends BaseAbstractControl<unknown>>(
  reducer: Reducer<T>,
  hub = HubFactory(),
): Observable<Action<T>> => {
  const { messages$ } = hub;
  ...
  const sourceForHub2$ = store$.pipe(
    ...
    mergeMap(([formChangeAction, action]) => {
      ...
      let controlsToCheck: BaseAbstractControl<unknown>[];
